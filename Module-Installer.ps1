...
function Move-ModuleFiles {
    ...
    $path = (Resolve-Path -Path "${ArchiveFolder}\*-${GitHubBranch}\$Module").Path
    ...
    if ( test-path "$DestFolder") { Remove-Item "$DestFolder" -Force -Recurse -Confirm $False}
...
Move-ModuleFiles -ArchiveFolder $archiveName -Module $moduleToLoad -DestFolder $moduleFolder -ModuleHash "$($moduleHash.Hash)" -GitHubBranch $Branch
